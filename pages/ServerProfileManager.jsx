...
const ServerProfileManager = () => {
  ...
  // Generate token (creates a server with token)
                ...
                {servers.map((server) => (
                  <div key={server.id} className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-xl font-bold truncate">{server.name}</h3>
                      <div className={`px-2 py-1 text-xs rounded-full ${
                        server.is_public ? 'bg-green-900 text-green-300' : 'bg-gray-700 text-gray-300'
                      }`}>
          ...
          <div>
            {tokens.length === 0 ? (
              ...
              <div className="space-y-6">
                {tokens.map((token) => (
                  <div key={token.id} className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg font-bold">Token for {token.table_name}</h3>
                        {token.record_name && (
                          <p className="text-gray-400">Record: {token.record_name}</p>
                        )}
                      </div>
                      <div className={`px-2 py-1 text-xs rounded-full ${
                        token.status === 'active' ? 'bg-green-900 text-green-300' : 'bg-gray-700 text-gray-300'
                      }`}>
                        {token.status}
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex space-x-3">
                        {token.permissions.read && (
                          <span className="px-2 py-1 bg-blue-900 text-blue-300 rounded text-xs">Read</span>
                        )}
                        {token.permissions.write && (
                          <span className="px-2 py-1 bg-green-900 text-green-300 rounded text-xs">Write</span>
                        )}
                        {token.permissions.delete && (
                          <span className="px-2 py-1 bg-red-900 text-red-300 rounded text-xs">Delete</span>
                        )}
                      </div>
                    </div>
          ...
          <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Specific UUID or leave blank for access to all records</p>
              </div>
